...
pstruct.fileName = "Fig2A_ConstBurgerScaffold_v";
pstruct.veAdd = 4;
pstruct.L_f0 = 35;
pstruct.var_fmpL = 2;
pstruct.lambda_kv0 = (pstruct.lambda_kv0 + pstruct.Phi_kv*pstruct.lambda_kv0)/2;
...
% % Save data
    ...
    save(resultsFilePath,"ode_options","pstruct","y_out","t_out","versionStatus","versionCommitHash")

% % Save plots
    % Length, L_T
    f1 = figure(1);
    plot(t_out./60,y_out(:,4)./pstruct.L_Tf)
    xlim([0 90])
    ylim([0 1.2])
    ylabel('Normalized furrow length (\mum)')
    xlabel('Time (min)')
    ...
    f4 = figure(4);
    plot(t_out./60,(ones(length(t_out),1).*lambda_kv)./pstruct.lambda_kv0)
    xlim([0 90])
    ...
    f5 = figure(5);
    plot(t_out./60,(ones(length(t_out),1).*zeta_kv)./pstruct.lambda_kv0)
    xlim([0 90])
    ...
    f6 = figure(6);
    plot(t_out./60,(ones(length(t_out),1).*zeta_md)./pstruct.lambda_kv0)
    xlim([0 90])
